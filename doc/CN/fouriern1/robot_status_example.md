...
**世界坐标系：** 世界坐标系建立在 Aurora 初始化时机器人基座中心的地面投影点。x、y 和 z 轴的方向与世界坐标系对齐。
...
try:
    while True:
        ...
        print(f"[{time.strftime('%H:%M:%S')}] 机器人状态:")
        print(f"  基座 RPY: [{rpy[0]:.3f}, {rpy[1]:.3f}, {rpy[2]:.3f}]")
        print(f"  基座速度: [{vel_world[0]:.3f}, {vel_world[1]:.3f}, {vel_world[2]:.3f}]")
        print(f"  接触: {['是' if p > 0.5 else '否' for p in contact_probs]}")
        print(f"  左机械臂关节: {len(left_manipulator_pos)} 自由度")
        print(f"  左机械臂末端位置: [{left_manipulator_pose[0]:.3f}, {left_manipulator_pose[1]:.3f}, {left_manipulator_pose[2]:.3f}]")
        print("-" * 60)
