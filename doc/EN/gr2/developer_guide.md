...
Before using fourier aurora client, please make sure **Aurora** is started. To initialze fourier aurora client, use `get_instance` function. The *domain_id* and *robot_name* arguments are mandatory for connection with Aurora. For GR-2, the *domain_id* is set to 123 by default, and *robot_name* should be "gr2". The *namespace* and *is_ros_compatible* options are reserved for future use.
