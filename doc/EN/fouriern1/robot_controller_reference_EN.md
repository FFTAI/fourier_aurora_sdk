# Fourier N-1 Robot Controller Reference
...
Aurora provides a set of controllers that can be used to control the robot's motion. Some universal controllers are provided in the `fourier-aurora` package, while others are provided in the expansion packages according to the robot configuration. Use `fourier-aurora-fouriern1` for Fourier N-1 robot.

For DDS reference, please refer to [DDS Interface Reference](../aurora_dds_reference_EN.md).

For Fourier N-1 joint limits, please refer to [Fourier N-1 Specs](./robot_specs_EN.md).
...
- Velocity commands (vx, vy, vyaw).
  ...
  - Range: vx [-0.5, 0.5], vy [-0.5, 0.5], vyaw [-1.0, 1.0]
