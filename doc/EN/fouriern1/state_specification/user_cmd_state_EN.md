...
Upon switching to **UserCmd State**, the user can send external joint position commands for all joints, and external config commands such as pd parameters. The robot will execute these commands and update its states accordingly.

## State specification

State name | Task name     | Joystick mapping | DDS mapping | Frequency
-----------|---------------|------------------|-------------|-------------
UserCmd    | UserCmdTask   | No               | 9           | 500Hz
...
**Avaliable Control Groups:** `Left_Leg`, `Right_Leg`, `Waist`, `Left_Manipulator`, `Right_Manipulator`, `Left_Hand`, `Right_Hand`  
